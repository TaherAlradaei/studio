
import type { Dictionary } from "./en";

export const arStrings: Dictionary = {
  header: {
    title: "نادي الميدان الرياضي (أكاديمية الميدان)",
    bookField: "احجز ملعب",
    academy: "انضم للأكاديمية",
    findTeam: "ابحث عن فريق",
    myBookings: "حجوزاتي",
    admin: "الإدارة",
    menu: "القائمة",
  },
  footer: {
    copyright: "© {year} نادي الميدان الرياضي. جميع الحقوق محفوظة.",
    address: "شارع صفر، حدة مدينة، صنعاء، اليمن",
    phone: "الهاتف: +967 736 333 328",
  },
  welcomePage: {
    welcomeTitle: "أهلاً بكم في نادي الميدان الرياضي",
    welcomeMessage: "وجهتكم الأولى لكرة القدم في صنعاء. احجزوا ملعبًا، انضموا إلى أكاديميتنا، وكونوا جزءًا من مجتمعنا.",
    fieldTitle: "ملاعب عالية الجودة",
    fieldDesc: "ملاعبنا مجهزة بأفضل المعايير لضمان أفضل تجربة لعب.",
    coachTitle: "تدريب احترافي",
    coachDesc: "مدربونا المعتمدون مكرسون لتطوير مهارات الجيل القادم.",
    alreadyMember: "عضو بالفعل في الأكاديمية؟",
    alreadyMemberDesc: "سجل الدخول إلى مساحتك الشخصية لمشاركة تقدمك والتواصل مع زملائك.",
    memberAreaButton: "منطقة الأعضاء",
    ourMissionTitle: "مهمتنا",
    ourMissionText: "مهمتنا هي توفير بيئة رياضية آمنة ومحفزة للشباب لتطوير مهاراتهم في كرة القدم، مع التركيز على الروح الرياضية والعمل الجماعي والنمو الشخصي.",
    ourVisionTitle: "رؤيتنا",
    ourVisionText: "رؤيتنا هي أن نكون الأكاديمية الرائدة لكرة القدم في المنطقة، معترف بها لتميزها في تدريب المواهب الشابة وتنمية قادة المستقبل داخل وخارج الملعب.",
    ourGoalsTitle: "أهدافنا",
    ourGoalsText: "نهدف إلى تقديم تدريب عالي الجودة، وتعزيز حب اللعبة، وتطوير لاعبين منضبطين ومحترمين يساهمون بشكل إيجابي في مجتمعهم.",
    galleryTitle: "معرض الصور",
    ourHistoryTitle: "تاريخنا",
    ourHistoryText: "تأسس في عام 1990 كأول ملعب لكرة القدم في مدينة صنعاء، ويفتخر نادي الميدان الرياضي بتاريخ غني في خدمة المجتمع وتعزيز حب اللعبة. على مر العقود، تطورنا من ملعب بسيط إلى منشأة حديثة، مكرسين جهودنا دائمًا لتوفير بيئة استثنائية للاعبين من جميع الأعمار.",
    captainSpeechTitle: "كلمة الكابتن",
    captainSpeechText: "الكابتن حافظ، مدرب معتمد مكرس لتنمية المواهب الجديدة، يؤمن بأن كل لاعب يمتلك إمكانات فريدة. 'مهمتي هي إشعال شغف كرة القدم في قلوب شبابنا. نحن لا ندرب اللاعبين فقط؛ بل نربي قادة المستقبل، ونغرس فيهم الانضباط والاحترام والسعي الدؤوب للتميز.'",
    managerWordTitle: "من مكتب المدير",
    managerWordText: "وهيب حميد يرى أن النادي هو مكان خاص وسيظل مميزًا لكل من يهتم بالانضمام إلى الأكاديمية أو حجز الملعب للعب. 'نادينا هو أكثر من مجرد ملعب؛ إنه مجتمع. نحن ملتزمون بتقديم تجربة لا مثيل لها، سواء كنت تطمح لتكون لاعبًا محترفًا أو تبحث فقط عن الاستمتاع بلعبة رائعة مع الأصدقاء. أهلاً بك في بيتك.'",
    sponsorsTitle: "الداعمون الرسميون",
    sponsorsText: "نفخر بدعم نخبة من المؤسسات الرائدة التي تشاركنا التزامنا تجاه المجتمع والرياضة. شراكتهم حيوية في مساعدتنا على رعاية الجيل القادم من الرياضيين.",
    latestNewsTitle: "آخر الأخبار",
    featuredNewsTitle: "فريق الأكاديمية يحقق الفوز في البطولة",
    featuredNewsDate: "16 أغسطس 2024",
    featuredNewsSummary: "أداء مذهل من فريقنا تحت 14 سنة أدى إلى فوز مثير بنتيجة 2-1، مما ضمن الحصول على كأس البطولة الإقليمية. جاء الهدف الحاسم في الدقيقة الأخيرة من الوقت الإضافي...",
    newsItem1Title: "الإعلان عن جدول تدريب جديد",
    newsItem1Date: "15 أغسطس 2024",
    newsItem2Title: "تجارب أداء مفتوحة لحراس المرمى",
    newsItem2Date: "14 أغسطس 2024",
    newsItem3Title: "ملخص يوم المرح المجتمعي",
    newsItem3Date: "12 أغسطس 2024",
    playerSpotlightTitle: "نجم الأسبوع",
    playerSpotlightName: "عادل محمد",
    playerSpotlightQuote: "'هذا النادي أكثر من مجرد فريق؛ إنه عائلة. الدعم من المدربين وزملائي يدفعني لأكون أفضل كل يوم.'",
    socialMediaTitle: "تابعوا رحلتنا",
    faqTitle: "الأسئلة الشائعة",
    faq1Question: "ما هي ساعات حجز الملعب؟",
    faq1Answer: "ملاعبنا متاحة للحجز من الساعة 7:00 صباحًا حتى 12:00 ظهرًا ومن الساعة 2:00 ظهرًا حتى 12:00 منتصف الليل كل يوم.",
    faq2Question: "ما هي الأعمار المؤهلة للأكاديمية؟",
    faq2Answer: "نقدم حاليًا برامج لفئتين عمريتين: تحت 10 سنوات (U-10) وتحت 14 سنة (U-14). يمكنك العثور على مزيد من التفاصيل والتسجيل في صفحة الأكاديمية الخاصة بنا.",
    faq3Question: "هل توفرون المعدات للحجوزات؟",
    faq3Answer: "نحن نوفر المرمى وملعبًا نظيفًا ومُصانًا جيدًا. يُتوقع من اللاعبين إحضار كراتهم الخاصة وأحذية مناسبة وأي معدات شخصية أخرى.",
    faq4Question: "كيف يمكنني دفع قيمة الحجز؟",
    faq4Answer: "بعد تأكيد حجزك، ستتلقى تعليمات للاتصال بإدارتنا لإنهاء عملية الدفع. نقبل الدفع النقدي وطرق الدفع المحلية.",
    academySectionTitle: "أكاديمية الميدان",
    academySectionDesc: "أكثر من مجرد لعبة، نحن نبني أبطالاً للحياة.",
    academyBenefit1Title: "طاقم تدريب محترف",
    academyBenefit1Desc: "تعلم من مدربين معتمدين لديهم خبرة في تطوير الشباب.",
    academyBenefit2Title: "برامج مخصصة حسب العمر",
    academyBenefit2Desc: "تدريب مخصص للفئات العمرية تحت 10 وتحت 14 سنة لتحقيق أقصى استفادة من المهارات.",
    academyBenefit3Title: "تنمية شاملة",
    academyBenefit3Desc: "نحن نركز على العمل الجماعي والانضباط والروح الرياضية، وليس فقط على المهارات الفنية.",
  },
  bookingPage: {
    title: "احجز ملعبك",
    description: "اتبع الخطوات البسيطة أدناه لحجز ملعب كرة القدم الخاص بك في أكاديمية الميدان.",
    selectDate: "اختر تاريخًا",
    selectDateDesc: "انقر على تاريخ لرؤية الأوقات المتاحة.",
    pricingTitle: "الأسعار الافتراضية",
    pricingDesc: "الأسعار التالية هي للساعة الواحدة وقد تختلف.",
    pricingTimeSlot: "الفترة الزمنية",
    pricingPriceYER: "السعر (ريال يمني)",
  },
  timeSlotPicker: {
    title: "اختر فترة زمنية",
    description: "اختر مدة زمنية ووقتًا متاحًا لحجزك في {date}.",
    durationLabel: "مدة الحجز",
    durationPlaceholder: "اختر المدة",
    oneHour: "ساعة واحدة",
    oneAndHalfHour: "ساعة ونصف",
    twoHours: "ساعتان",
  },
  bookingForm: {
    title: "أدخل بياناتك",
    titleConfirm: "تأكيد تفاصيل الحجز",
    bookingFor: "الحجز بتاريخ {date} في الساعة {time} لمدة {duration} ساعة (ساعات).",
    nameLabel: "الاسم الكامل",
    namePlaceholder: "مثال: وهيب حميد",
    phoneLabel: "رقم الهاتف",
    phonePlaceholder: "مثال: 736333328",
    termsLabel: "أوافق على الشروط والأحكام",
    requestButton: "تأكيد طلب الحجز",
    bookingAs: "الحجز باسم:",
    validation: {
      nameMin: "يجب أن يتكون الاسم من حرفين على الأقل.",
      phoneFormat: "الرجاء إدخال رقم هاتف صحيح.",
      termsRequired: "يجب عليك قبول الشروط والأحكام.",
    },
  },
  bookingsPage: {
    title: "حجوزاتي",
    description: "هذه قائمة بجميع حجوزاتك القادمة في أكاديمية الميدان.",
  },
  bookingHistoryTable: {
    date: "التاريخ",
    time: "الوقت",
    duration: "المدة",
    name: "الاسم",
    price: "السعر",
    status: "الحالة",
    actions: "الإجراءات",
    statusPending: "في انتظار موافقة الإدارة",
    statusAwaitingConfirmation: "في انتظار موافقتك",
    statusConfirmed: "مؤكد",
    statusCancelled: "ملغي",
    priceTBD: "السعر قيد التأكيد",
    priceEstimated: "تقريبا. {price} ريال يمني",
    noBookings: "ليس لديك حجوزات قادمة.",
    durationValue: "{duration} ساعة (ساعات)",
    paymentDialogTitle: "تم تأكيد الحجز وبانتظار الدفع",
    paymentDialogDescription: "حجزك بتاريخ {date} في الساعة {time} يتطلب الدفع المسبق. يرجى اتباع التعليمات أدناه لإتمام عملية الدفع وتأمين حجزك.",
    paymentDialogInstructions: "يرجى الاتصال بالإدارة على الرقم <a href=\"tel:{phone}\" class=\"text-primary underline\">{phone}</a> لترتيب عملية الدفع.",
  },
  adminPage: {
    title: "مساعد الجدولة الذكي",
    description: "استخدم أداتنا المدعومة بالذكاء الاصطناعي لتحليل أنماط الحجز وتلقي توصيات للجدولة المثلى لزيادة استخدام المنشأة.",
    dataCardTitle: "بيانات الحجز",
    dataCardDescription: "أدخل بيانات الحجز التاريخية بتنسيق JSON. يمكنك استخدام بيانات الحجز الحالية للتطبيق كمثال.",
    dataPlaceholder: "الصق بيانات الحجز هنا...",
    analyzeButton: "تحليل الأنماط",
    analyzingButton: "جارٍ التحليل...",
    useMockButton: "استخدام بيانات حجز التطبيق",
    errorTitle: "خطأ",
    recommendationsTitle: "توصيات الجدولة",
    errorEmpty: "تعذر الحصول على توصيات. كانت النتيجة فارغة.",
    errorAnalyzing: "حدث خطأ أثناء تحليل بيانات الحجز.",
    bookingManagementCardTitle: "إدارة الحجوزات",
    bookingManagementCardDescription: "عرض وإدارة جميع حجوزات العملاء. تأكيد الطلبات المعلقة وتحديد الأسعار النهائية.",
    customer: "العميل",
    duration: "المدة",
    price: "السعر",
    status: "الحالة",
    actions: "الإجراءات",
    setPriceButton: "تحديد السعر",
    confirmButton: "تأكيد",
    cancel: "إلغاء",
    edit: "تعديل",
    noBookings: "لا توجد حجوزات لعرضها.",
    confirmDialogTitle: "تحديد السعر وإرسال عرض",
    confirmDialogDescription: "حدد السعر للساعة لهذا الحجز. سيتم حساب السعر الإجمالي وإرساله للعميل للتأكيد.",
    priceLabel: "السعر للساعة (ريال يمني)",
    confirmSubmitButton: "إرسال عرض السعر",
    manageAvailabilityCardTitle: "إدارة توفر الملعب",
    manageAvailabilityCardDescription: "أنشئ حجزًا مؤكدًا جديدًا، أو قم بحظر أو إلغاء حظر فترات زمنية مباشرة في التقويم.",
    block: "حظر",
    unblock: "إلغاء الحظر",
    booked: "محجوز",
    blocked: "محظور",
    invalidPriceToastTitle: "سعر غير صالح",
    invalidPriceToastDesc: "الرجاء إدخال رقم موجب صالح للسعر بالساعة.",
    paymentInstructionsCardTitle: "تعليمات الدفع",
    paymentInstructionsCardDescription: "سيتم عرض هذه الرسالة للعملاء بعد تأكيد الحجز. قم بإرشادهم حول كيفية إتمام عملية الدفع.",
    paymentInstructionsLabel: "رسالة تعليمات الدفع",
    saveButton: "حفظ",
    savingButton: "جارٍ الحفظ...",
    instructionsSavedToastTitle: "تم حفظ التعليمات",
    instructionsSavedToastDesc: "تم تحديث تعليمات الدفع بنجاح.",
    manageBackgroundsCardTitle: "إدارة صور الخلفية",
    manageBackgroundsCardDescription: "استبدل صور الخلفية المتحركة المعروضة في جميع أنحاء التطبيق. قم بتحميل صور جديدة وقدم تلميحات وصفية للذكاء الاصطناعي.",
    replaceImageButton: "استبدال",
    imageHintLabel: "تلميح الصورة (للذكاء الاصطناعي)",
    imageHintPlaceholder: "مثال: هدف كرة قدم",
    backgroundUpdatedToastTitle: "تم تحديث الخلفية",
    backgroundUpdatedToastDesc: "تم تحديث صورة الخلفية بنجاح.",
    manageLogoCardTitle: "إدارة شعار التطبيق",
    manageLogoCardDescription: "استبدل شعار التطبيق المعروض في الترويسة.",
    replaceLogoButton: "استبدال الشعار",
    logoUpdatedToastTitle: "تم تحديث الشعار",
    logoUpdatedToastDesc: "تم تحديث شعار التطبيق بنجاح.",
    filterByDate: "تصفية حسب تاريخ البدء",
    filterByRange: "تصفية حسب النطاق",
    day: "يوم",
    week: "أسبوع",
    month: "شهر",
    filterDescription: "سيعرض الجدول أدناه الحجوزات للتاريخ والنطاق المحددين.",
    noBookingsInView: "لم يتم العثور على حجوزات للفترة المحددة.",
    calendarView: "عرض التقويم",
    oneMonth: "شهر واحد",
    twoMonths: "شهران",
    bookingDetailsTitle: "تفاصيل الحجز",
    bookingDetailsDescription: "تفاصيل الحجز في الساعة {time}.",
    bookingDetailsName: "الاسم:",
    bookingDetailsPhone: "الهاتف:",
    academyRegistrationsTitle: "تسجيلات الأكاديمية",
    academyRegistrationsDesc: "مراجعة وإدارة طلبات الالتحاق بأكاديمية الناشئين.",
    talentName: "اسم الموهبة",
    age: "العمر",
    ageGroup: "الفئة العمرية",
    parentContact: "ولي الأمر",
    accessCode: "رمز الدخول",
    noRegistrations: "لا توجد طلبات تسجيل في الأكاديمية.",
    dashboardTab: "لوحة التحكم",
    settingsTab: "الإعدادات",
    exportsTab: "التصدير",
    usersTab: "المستخدمون",
    newsTab: "الأخبار",
    layoutTabDescription: "قم بتخصيص تخطيط لوحة التحكم الخاصة بك.",
    reorderComingSoon: "ستتوفر إمكانية إعادة ترتيب الأقسام قريبًا.",
    userManagementTitle: "إدارة المستخدمين",
    userManagementDesc: "إدارة أدوار وأذونات المستخدمين. منح صلاحيات المسؤول أو العميل الموثوق.",
    addTrustedCustomerLabel: "إضافة عميل موثوق جديد",
    trustedCustomerNamePlaceholder: "اسم العميل الكامل",
    addCustomerButton: "إضافة",
    trustedCustomersListTitle: "قائمة العملاء الموثوقين الحالية",
    noTrustedCustomers: "لا يوجد عملاء موثوقون حتى الآن.",
    trustedCustomerAddedToastTitle: "تمت إضافة عميل موثوق",
    trustedCustomerAddedToastDesc: "تمت إضافة المستخدم إلى قائمة العملاء الموثوقين.",
    trustedCustomerRemovedToastTitle: "تمت إزالة عميل موثوق",
    trustedCustomerRemovedToastDesc: "تمت إزالة المستخدم من قائمة العملاء الموثوقين.",
    trustedStatusLabel: "موثوق",
    adminStatusLabel: "مسؤول",
    adminAddedToastTitle: "تمت إضافة مسؤول",
    adminAddedToastDesc: "تم منح المستخدم صلاحيات المسؤول.",
    adminRemovedToastTitle: "تمت إزالة مسؤول",
    adminRemovedToastDesc: "تم إلغاء صلاحيات المسؤول للمستخدم.",
    addMemberCardTitle: "إضافة عضو جديد في الأكاديمية",
    addMemberCardDesc: "أدخل تفاصيل عضو جديد في الأكاديمية يدويًا. سيتم قبوله تلقائيًا وإنشاء رمز دخول له.",
    addMemberButton: "إضافة عضو",
    memberAddedSuccessTitle: "تمت إضافة العضو بنجاح",
    memberAddedSuccessDesc: "تمت إضافة {name} إلى الأكاديمية بنجاح.",
    manualBookingTitle: "إنشاء حجز مؤكد",
    manualBookingDescription: "أدخل تفاصيل العميل للحجز بتاريخ {date} في الساعة {time}.",
    manualBookingError: "الرجاء إدخال الاسم ورقم الهاتف لإنشاء حجز.",
    makeRecurringButton: "جعله متكررًا",
    recurringBookingTitle: "إنشاء حجز أسبوعي متكرر",
    recurringBookingDesc: "كرر الحجز لـ {name} في الساعة {time} يوم {day} كل أسبوع للشهر التالي.",
    recurringBookingSuccessTitle: "تم إنشاء الحجوزات المتكررة",
    recurringBookingSuccessDesc: "تم إنشاء 4 حجوزات أسبوعية جديدة لـ {name} بنجاح.",
    manageWelcomePageCardTitle: "إدارة صور صفحة الترحيب",
    manageWelcomePageCardDescription: "تحرير الصور المعروضة في صفحة الترحيب العامة.",
    welcomePageFieldImageLabel: "صورة الملعب",
    welcomePageCoachImageLabel: "صورة المدرب",
    welcomePageManagerImageLabel: "صورة المدير",
    welcomePageContentUpdatedTitle: "تم تحديث المحتوى",
    welcomePageTextUpdatedDesc: "تم تحديث نصوص صفحة الترحيب بنجاح.",
    welcomePageImageUpdatedDesc: "تم تحديث صورة صفحة الترحيب بنجاح.",
    securitySettingsCardTitle: "إعدادات الأمان",
    securitySettingsCardDesc: "إدارة رمز الدخول الخاص بالمسؤول للوصول إلى لوحة التحكم.",
    securityCurrentCodeLabel: "رمز الدخول الحالي للمسؤول",
    securityNewCodeLabel: "رمز الدخول الجديد للمسؤول",
    securityNewCodePlaceholder: "أدخل الرمز الجديد هنا",
    securityChangeCodeButton: "تغيير الرمز",
    securityCodeUpdatedTitle: "تم تحديث الرمز",
    securityCodeUpdatedDesc: "تم تحديث رمز الدخول الخاص بالمسؤول بنجاح.",
    securityCodeEmpty: "لا يمكن أن يكون رمز الدخول الجديد فارغًا.",
    exportButton: "تصدير",
    exportBookingsTitle: "تصدير الحجوزات",
    exportBookingsDesc: "تنزيل ملف CSV لبيانات الحجوزات. اختياريًا، قم بالتصفية حسب نطاق تاريخ محدد.",
    exportByDateRange: "تصفية حسب نطاق التاريخ (اختياري)",
    exportStartDate: "تاريخ البدء",
    exportEndDate: "تاريخ الانتهاء",
    exportRegistrationsTitle: "تصدير تسجيلات الأكاديمية",
    exportRegistrationsDesc: "تنزيل ملف CSV لجميع بيانات تسجيل الأكاديمية.",
    archiveButton: "أرشفة",
    statusArchived: "مؤرشف",
    archivedRegistrationsTitle: "التسجيلات المؤرشفة",
    archivedRegistrationsDesc: "هذه قائمة بالأعضاء من المواسم السابقة أو الذين تمت أرشفتهم يدويًا.",
    noArchivedRegistrations: "لا توجد تسجيلات مؤرشفة.",
    newsManagement: {
      title: "إدارة الأخبار",
      description: "إنشاء وتعديل وحذف المقالات الإخبارية للصفحة الرئيسية.",
      createNew: "إنشاء مقال جديد",
      editArticle: "تعديل المقال",
      noArticles: "لا توجد مقالات إخبارية حتى الآن. قم بإنشاء أول مقال!",
      titleEN: "العنوان (باللغة الإنجليزية)",
      titleAR: "العنوان (باللغة العربية)",
      summaryEN: "الملخص (باللغة الإنجليزية)",
      summaryAR: "الملخص (باللغة العربية)",
      image: "الصورة المميزة",
      imageDesc: "سيتم عرض هذه الصورة مع المقال.",
      publish: "نشر",
      update: "تحديث",
      deleteConfirmTitle: "هل أنت متأكد؟",
      deleteConfirmDesc: "سيؤدي هذا إلى حذف المقال الإخباري بشكل دائم. لا يمكن التراجع عن هذا الإجراء.",
      articleCreated: "تم إنشاء المقال الإخباري بنجاح.",
      articleUpdated: "تم تحديث المقال الإخباري بنجاح.",
      articleDeleted: "تم حذف المقال الإخباري بنجاح.",
      validation: {
        titleEN: "العنوان باللغة الإنجليزية مطلوب.",
        titleAR: "العنوان باللغة العربية مطلوب.",
        summaryEN: "الملخص باللغة الإنجليزية مطلوب.",
        summaryAR: "الملخص باللغة العربية مطلوب.",
      }
    },
  },
  academyPage: {
    title: "انضم إلى الأكاديمية",
    description: "سجل موهبتك الشابة للانضمام إلى أكاديمية الميدان لكرة القدم. لدينا برامج للفئات العمرية تحت 10 سنوات وتحت 14 سنة.",
    formTitle: "استمارة التسجيل",
    formDescription: "يرجى ملء النموذج أدناه. سيقوم فريقنا بمراجعة الطلب والاتصال بك.",
    parentNameLabel: "اسم ولي الأمر",
    parentNamePlaceholder: "مثال: وهيب حميد",
    talentNameLabel: "اسم الموهبة الكامل",
    talentNamePlaceholder: "مثال: محمد وهيب حميد",
    birthDateLabel: "تاريخ ميلاد الموهبة",
    ageGroupLabel: "الفئة العمرية",
    ageGroupPlaceholder: "اختر فئة عمرية",
    ageGroupU10: "تحت 10 سنوات",
    ageGroupU14: "تحت 14 سنة",
    submitButton: "إرسال الطلب",
    statusPending: "قيد المراجعة",
    statusAccepted: "مقبول",
    statusRejected: "مرفوض",
    toastSuccessTitle: "تم إرسال الطلب",
    toastSuccessDesc: "تم إرسال طلب التسجيل لـ {name} بنجاح. سنتصل بك قريباً.",
    validation: {
      parentNameMin: "يجب أن يتكون اسم ولي الأمر من حرفين على الأقل.",
      talentNameMin: "يجب أن يتكون اسم الموهبة من حرفين على الأقل.",
      birthDateInvalid: "الرجاء إدخال تاريخ ميلاد صحيح بالتنسيق YYYY-MM-DD.",
    },
  },
  memberArea: {
    title: "ساحة أعضاء الأكاديمية",
    description: "أدخل رمز الدخول الخاص بك للوصول إلى مساحتك الشخصية.",
    accessCodePlaceholder: "أدخل الرمز هنا",
    enterButton: "دخول",
    invalidCodeTitle: "رمز غير صالح",
    invalidCodeDesc: "الرمز الذي أدخلته غير صحيح. يرجى المحاولة مرة أخرى.",
    welcome: "أهلاً بك، {name}!",
    welcomeDesc: "هذه هي مساحتكم المشتركة. شاركوا قصصكم مع الأكاديمية!",
    addPostTitle: "أضف قصة جديدة",
    addPostDesc: "شارك تجربة، ذكرى، أو حتى صورة!",
    storyPlaceholder: "اكتب قصتك هنا...",
    addPostButton: "أضف قصة",
    addImage: "أضف صورة",
    changeImage: "تغيير الصورة",
    galleryTitle: "قصص الأكاديمية",
    noPosts: "لا توجد أي قصص حتى الآن. أضف أول قصة!",
    postAddedSuccess: "تمت إضافة قصتك بنجاح!",
    postEmpty: "لا يمكن أن يكون المنشور فارغًا. الرجاء كتابة قصة أو إضافة صورة.",
    addCommentPlaceholder: "أضف تعليقًا...",
    logout: "تسجيل الخروج",
  },
  findATeamPage: {
    title: "ابحث عن فريق",
    description: "ليس لديك فريق؟ لا مشكلة! سجل اهتمامك أدناه، وسنساعدك في العثور على فريق للعب معه.",
    formTitle: "استمارة التسجيل",
    formDescription: "املأ بياناتك، وسنتواصل معك عند توفر فرصة للانضمام إلى فريق.",
    positionLabel: "مركز اللعب المفضل",
    positionPlaceholder: "اختر مركزك",
    positionGoalkeeper: "حارس مرمى",
    positionDefender: "مدافع",
    positionMidfielder: "لاعب وسط",
    positionForward: "مهاجم",
    positionAny: "أي مركز",
    availabilityLabel: "مواعيدك المتاحة",
    availabilityPlaceholder: "مثال: أيام الأسبوع بعد الساعة 5 مساءً، وعطلات نهاية الأسبوع",
    availabilityDesc: "اذكر الأيام والأوقات التي تكون متاحًا فيها للعب عادةً.",
    submitButton: "إرسال التسجيل",
    toastSuccessTitle: "تم إرسال التسجيل",
    toastSuccessDesc: "لقد سجلنا اهتمامك. سيتم الآن عرض قائمتك للاعبين الآخرين.",
    validation: {
      availabilityMin: "يرجى تقديم بعض التفاصيل حول مواعيدك المتاحة.",
    },
    playerListTitle: "لاعبون يبحثون عن فريق",
    playerListDesc: "هذه قائمة باللاعبين الآخرين الذين يبحثون عن فرق. تواصل معهم لتشكيل فريق!",
    noPlayers: "لا يوجد حاليًا لاعبون يبحثون عن فرق. كن أول من ينضم إلى القائمة!",
    leaveListButton: "مغادرة القائمة",
    leaveListConfirmTitle: "هل أنت متأكد؟",
    leaveListConfirmDesc: "سيؤدي هذا إلى إزالة اسمك من القائمة العامة. يمكنك دائمًا التسجيل مرة أخرى لاحقًا.",
    leaveListSuccess: "تمت إزالتك من القائمة بنجاح.",
    managementMessageTitle: "هيا نكوّن فريقًا!",
    managementMessageDesc: "إدارة الميدان تشجعكم على التواصل مع زملائكم اللاعبين. تواصلوا، شكلوا فريقًا، وتقاسموا التكاليف. ليس لديكم ما تخسرونه، بل مباراة رائعة ستكسبونها!"
  },
  toasts: {
    bookingConfirmedTitle: "تم تأكيد الحجز!",
    bookingConfirmedDesc: "تم تأكيد حجزك بتاريخ {date} في الساعة {time}.",
    bookingConfirmedDescAdmin: "تم تأكيد الحجز لـ {name}.",
    bookingPendingTitle: "تم إرسال طلب الحجز",
    bookingPendingDesc: "طلبك بتاريخ {date} في الساعة {time} قيد انتظار التأكيد من الإدارة.",
    bookingUpdateTitle: "تم تحديث الحجز",
    bookingUpdateDesc: "تم تحديث الحجز لـ {name}.",
    priceQuoteSent: "تم إرسال عرض السعر إلى {name}.",
    bookingCancelled: "تم إلغاء الحجز.",
    slotUnavailableTitle: "الفترة الزمنية لم تعد متاحة",
    slotUnavailableDesc: "تم تأكيد هذه الفترة الزمنية للتو من قبل مستخدم آخر. يرجى اختيار وقت مختلف.",
    registrationUpdateTitle: "تم تحديث التسجيل",
    registrationUpdateDesc: "تم {status} طلب تسجيل {name}.",
    bookingAwaitingConfirmationTitle: "طلب الحجز قيد الانتظار",
    bookingAwaitingConfirmationDesc: "تم تقديم طلب الحجز الخاص بك. يرجى الانتقال إلى 'حجوزاتي' للتأكيد.",
  },
  auth: {
      errorTitle: "خطأ في المصادقة",
      notLoggedInTitle: "غير مسجل الدخول",
      notLoggedInDesc: "يجب عليك تسجيل الدخول لتنفيذ هذا الإجراء.",
      createAccountTitle: "إنشاء حساب / تسجيل الدخول",
      createAccountDesc: "سجل الدخول للمتابعة. يمكن للمسؤولين الدخول من منطقة الأعضاء.",
      getStartedTitle: "ابدأ الآن",
      getStartedDesc: "أنشئ حسابك لبدء حجز الملاعب والانضمام إلى مجتمعنا.",
      createAccountAction: "إنشاء حساب",
      login: "تسجيل الدخول",
      adminLogin: "دخول المسؤول/العميل الموثوق",
      adminLoginTitle: "دخول المسؤول والعميل الموثوق",
      adminLoginDesc: "استخدم حساب جوجل المسجل لديك للوصول إلى الميزات المخصصة.",
      logout: "تسجيل الخروج",
      continue: "متابعة",
      continueWithGoogle: "المتابعة باستخدام جوجل",
      completeProfileTitle: "أكمل ملفك الشخصي",
      completeProfileDesc: "يرجى تقديم اسمك ورقم هاتفك للمتابعة.",
      detailsUpdatedTitle: "تم تحديث الملف الشخصي",
      detailsUpdatedDesc: "تم حفظ بياناتك بنجاح.",
      anonBookingTitle: "لحظة من فضلك!",
      anonBookingDesc: "يتم حفظ حجوزاتك على هذا الجهاز. سجل الدخول باستخدام جوجل للوصول إليها من أي مكان والحصول على ميزات إضافية."
  },
  actions: {
      accept: "قبول",
      decline: "رفض",
      ok: "موافق",
  }
};
